...
{
    [BurstCompile]
    public struct RotationSerializer
    {
        ...
        {
            ref Rotation comp = ref GhostComponentSerializer.TypeCast<Rotation>(compPtr);
        ...
        static void CopyFromSnapshot(IntPtr compPtr, IntPtr dataPtr, int offset)
        {
			...
			comp.Value = math.slerp(before.Value, after.Value, dataAtTick.InterpolationFactor);
        }
        ...
        {
            ref Snapshot snapshot = ref GhostComponentSerializer.TypeCast<Snapshot>(dataPtr);
			writer.WritePackedQuaternion(snapshot.Value, compressionModel);
        }
        ...
        {
            ref Snapshot snapshot = ref GhostComponentSerializer.TypeCast<Snapshot>(dataPtr);
			snapshot.Value = reader.ReadPackedQuaternion(compressionModel);
        }
    }
}